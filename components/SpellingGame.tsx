...
const SpellingGame: React.FC<SpellingGameProps> = ({ words, onFinish }) => {
    ...
    const announceWord = async () => {
        if (audioContextRef.current && currentWord) {
            ...
            try {
                const response = await fetch(`/audio/${currentWord}.mp3`);
                if (response.ok) {
                    ...
                    console.error(`Audio file not found for: ${currentWord}`);
                }
            } catch (error) {
    ...
    return (
        <div className="max-w-2xl mx-auto p-8 bg-white rounded-2xl shadow-xl border border-slate-100 flex flex-col items-center text-center">
            ...
            <button
                ...
                className={`group mb-8 p-6 rounded-full transition-all ${isAudioLoading
            ...
            <form onSubmit={handleSubmit} className="w-full">
                <input
                    ...
                    className={`w-full text-center text-3xl font-bold p-4 border-b-4 outline-none transition-all uppercase tracking-widest ${feedback === 'correct' ? 'border-green-500 text-green-600' :
